...
import { displayPokemonDe, resolvePokemonToEn, searchPokemon } from "@/lib/pokedex";
...
function ballModifier(ball: BallKey, opts: { isDark: boolean; firstTurn: boolean; alreadyCaught: boolean }) {
  switch (ball) {
    case "poke":
      return 1;
    case "great":
...
export function CatchRateLauncher() {
  ...
  useEffect(() => {
    ...
    async function loadPokemon() {
      ...
      try {
        const pokemonRes = await fetch(`https://pokeapi.co/api/v2/pokemon/${resolvedEn}`);
    ...
    <>
      ...
      {open ? (
        <div className="fixed inset-0 z-[80] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-4xl rounded-3xl border border-[rgba(212,175,55,0.45)] bg-zinc-950/95 shadow-[0_0_40px_rgba(0,0,0,0.6)] overflow-hidden fade-in-up soft-shimmer">
            <div className="flex items-start justify-between gap-4 border-b border-[rgba(212,175,55,0.18)] px-5 py-4">
              ...
              <button
                ...
                className="rounded-xl border border-zinc-700 bg-zinc-900/70 px-3 py-2 text-zinc-200 hover:bg-zinc-800"
              >
                ✕
              </button>
            </div>
              ...
              <div className="space-y-4">
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div className="rounded-2xl border border-[rgba(212,175,55,0.18)] bg-black/15 px-4 py-3">
                    <div className="text-sm font-medium text-zinc-300">Spezialoptionen</div>
                ...
                <div className="rounded-2xl border border-[rgba(212,175,55,0.18)] bg-black/15 p-4">
                  ...
                  <div className="mt-4 text-xs leading-6 text-zinc-400">
                    Hinweis: Das Tool rechnet global für dein aktuell gewähltes Pokémon
                    und ist damit ideal für Randomizer-Runs. Für SoulSilver wird mit
                    Gen-4-Catch-Mechanics gerechnet.
                  </div>
                </div>
              </div>
