...
function typeRing(type1?: string | null) {
  ...
  return TYPE_META[key]?.accent ?? "ring-zinc-700/30";
}
      ...
      <div className="mt-4 grid grid-cols-3 gap-3">
        {slots.map((e, i) => (
          <div
            key={i}
            className={[
              "rounded-xl border p-3 min-h-[92px] transition",
              e ? cardBg(e.status) : "bg-zinc-950/30 border-zinc-800",
              e?.type1 ? "ring-1" : "",
              e?.type1 ? typeRing(e.type1) : "",
            ].join(" ")}
          >
            {e ? (
              <div className="flex gap-3">
                <div className="h-12 w-12 rounded-lg bg-zinc-950/40 border border-zinc-800 flex items-center justify-center overflow-hidden">
                  {e.sprite_url ? (
                    ...
                    <img src={e.sprite_url} alt="" className="h-10 w-10" />
                  ) : (
